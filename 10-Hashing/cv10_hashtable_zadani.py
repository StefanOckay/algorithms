...
def insert_hashtable(hashtable, key, data):
    ...
    # TODO
    insert_linked_list(hashtable.table[hash(pair.key)], pair)
...
def get_hashtable(hashtable, key):
    ...
    """
    # TODO
    tmp = search_linked_list(hashtable.table[hash(key)], key)
    if tmp is None:
        return None
...
def remove_hashtable(hashtable, key):
    ...
    # TODO
    to_be_deleted = search_linked_list(hashtable.table[hash(key)], key)
    delete_linked_list(hashtable.table[hash(key)], to_be_deleted)


def keys_hashtable(hashtable):
    """Vrati seznam vsech klicu v tabulce."""
    # TODO
    lenh = len(hashtable.table)
    keys_list = []
    for i in range(lenh):
        current_list = hashtable.table[i]
        current_node = current_list.first
        while current_node is not None:
            keys_list.append(current_node.pair.key)
...
def values_hashtable(hashtable):
    ...
    for i in range(lenh):
        ...
        current_node = current_list.first
        while current_node is not None:
            data_list.append(current_node.pair.data)
            current_node = current_node.next
    return data_list
